...
public class HTMLLauncher implements HTMLResultsListener {
	...
	private String runHTMLSuite(String browser, String browserURL, String suiteURL, File outputFile,
		...
		log.info("Output file for reports : " + outputFile.getAbsolutePath());

		int indexEndDir = outputFile.getAbsolutePath().lastIndexOf(File.separator);

		if (indexEndDir > 0) {
			this.outputFolder = outputFile.getAbsolutePath().substring(0, indexEndDir);
			log.info("Output directory : " + outputFolder);
		}
		...
		log.info("Waiting test suite during " + (timeoutInMs / 1000) + "s");
		...
		if (results == null) {
			log.info("Suite timeout !!! after" + (timeoutInMs / 1000) + "s");
	...
	public String runHTMLSuite(String browser, String browserURL, File suiteFile, File outputFile,
		...
		this.outputFolder = outputFile.getParent();
		...
		String suiteURL;
		if (browser.startsWith("*chrome") || browser.startsWith("*firefox") || browser.startsWith("*iehta")
				|| browser.startsWith("*iexplore")) {
			suiteURL = "http://localhost:" + remoteControl.getConfiguration().getPortDriversShouldContact()
					+ "/selenium-server/tests/" + urlEncodedSuiteFilename;
		} else {
			suiteURL = Urls.toProtocolHostAndPort(browserURL) + "/selenium-server/tests/" + urlEncodedSuiteFilename;
		}
		return runHTMLSuite(browser, browserURL, suiteURL, outputFile, timeoutInSeconds, multiWindow, "info");
	}
	...
	public static int mainInt(String... args) throws Exception {
		...
		if (args.length == 4) {
			...
			browsers = new String[] { BrowserType.FIREFOX, BrowserType.IEXPLORE_PROXY, BrowserType.OPERA,
					BrowserType.CHROME };
		} else {
	...
	public void processStoreReport(String testSuiteName, String testCaseName, String timeStamp) {
		if (outputFolder != null) {
			File output_ = new File(outputFolder);
			// log.log(Level.INFO, output_.getAbsolutePath());

			File tmpReportDirectory = new File("/tmp/screenshotOnFailure/" + timeStamp.substring(0, 8));
			log.log(Level.INFO, "Moving snapshots and logs " + tmpReportDirectory.getAbsolutePath() + " to : "
					+ outputFolder);

			String domScreenshot = timeStamp + "-" + testSuiteName + "-" + testCaseName + "_DOM.html";
			String pngScreenshot = timeStamp + "-" + testSuiteName + "-" + testCaseName + ".png";
			String logs = timeStamp + "-" + testSuiteName + "-" + testCaseName + ".txt";
			try {
				File screenshotSrcFile = new File(tmpReportDirectory, domScreenshot);
				File screenshotImgSrcFile = new File(tmpReportDirectory, pngScreenshot);
				File screenshotStbLogSrcFile = new File(tmpReportDirectory, logs);

				if (screenshotSrcFile.exists()) {
					Files.move(screenshotSrcFile, new File(output_, domScreenshot));
				}
				if (screenshotImgSrcFile.exists()) {
					Files.move(screenshotImgSrcFile, new File(output_, pngScreenshot));
				}
				if (screenshotStbLogSrcFile.exists()) {
					Files.move(screenshotStbLogSrcFile, new File(output_, logs));
				}
			} catch (Exception e) {
				log.log(Level.SEVERE, "Unable to copy error report files " + e.getMessage());
				e.printStackTrace();
			}
		} else {
			log.log(Level.SEVERE, "Unable to find target directory for reports");
		}
